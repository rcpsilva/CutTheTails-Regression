...
'''
    ...
    No final da execução deste script, será gerado um arquivo csv de nome "baseline_results.csv" no mesmo nível que este script.
...
def select_model_classifier(name):
    ...
    if name == 'XGBoost':
        model = XGBRegressor(n_estimators=100, max_leaves=0, random_state = 0)
...
for i in Models:
    for j in Bases:
        for l in Replicas:
            ...
            model = select_model_classifier(i)
            ...
            _X_train, _X_test, y_train, y_test = train_test_split(df, df[target], test_size=0.2)
            X_train = _X_train[features].to_numpy()
            X_test = _X_test[features].to_numpy()
            y_train, y_test = y_train.to_numpy(), y_test.to_numpy()

            #construindo os modelo baseline
            model.fit(X_train,y_train)

            #predicting...
            y_base = model.predict(X_test)

            Mape_test = mean_absolute_percentage_error(y_base,y_test)
        ...
        df_output.to_csv('baseline_results.csv', encoding='utf-8', index=False)